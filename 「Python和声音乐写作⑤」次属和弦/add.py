...
while i:
    ...
    if j == 0:
        ...
        j += 1
    chord_loc = int(input('想在哪一个和弦的后面应用五度圈(不能是最后一个):\n'))
...
j=0
while i:
    judge = int(input('是否需要添加属七和弦？输入1为是，0为否:\n'))
    if judge == 0:
        break
    if j == 0:
        corresponding_time = [4,4,4,4]
        j += 1
    chord_loc = int(input('想在哪一个和弦的后面应用属七和弦(不能是最后一个):\n'))
    dominant(chord_list, name_list, chord_loc)
    original_time = corresponding_time.pop(chord_loc - 1)
    corresponding_time.insert(chord_loc - 1, 1)
    corresponding_time.insert(chord_loc - 1, original_time - 1)
    print ("新的和弦进行是:\n", chord_list)
